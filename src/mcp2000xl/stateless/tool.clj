(ns mcp2000xl.stateless.tool
  "Tool specifications for stateless MCP servers"
...
(defn create-tool-specification
  "Create stateless MCP tool specification from clojure data / function / malli schemas.
   
   This is for stateless servers - the handler is synchronous and takes McpTransportContext
   instead of McpExchange.
   ...
   - :handler - Function (fn [context {:keys [args]}] ...) that returns result map
            ...
            (try
              (let [request-data (McpSchema$CallToolRequest/.arguments request)
                ...
                (if-some [explanation (request-explainer coerced-request-data)]
                  ...
                  (let [response-data (handler context coerced-request-data)
